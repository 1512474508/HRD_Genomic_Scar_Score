#Author Samir Lal 
# La Jolla Pfizer Inc Constructed 2017 
# This code calculates all of the HRD Statistics including HRD-LOH, TAI and LST per sample
path <- "/Users/LALS07/Desktop/"

load(paste(path,"sysdata.rda",sep="/"))

calculate_scar_score <- function (seg, reference = "grch38", seqz = FALSE, ploidy = NULL, 
                                  sizelimitLOH = 1.5e+07, outputdir = NULL) 
{
  ...
  if (seqz == TRUE) {
    ...
    seg <- read.table(seg, header = T, check.names = F, 
  ...
  names(HRDresulst) <- c("HRD", colnames(res_ai)[1], "LST", 
                         "HRD-sum")
...
{
  seg <- seg[!seg[, 2] %in% c(paste("chr", c("X", "Y", "x", 
                                             "y", 23, 24), sep = ""), c("X", "Y", "x", "y", 23, 24)), 
             ]
  seg[, 1] <- as.character(seg[, 1])
  if (!all(seg[, 8] <= seg[, 7])) {
    tmp <- seg
    seg[tmp[, 8] > tmp[, 7], 7] <- tmp[tmp[, 8] > tmp[, 7], 
                                       8]
    seg[tmp[, 8] > tmp[, 7], 8] <- tmp[tmp[, 8] > tmp[, 7], 
                                       7]
  }
...
calc.hrd<- function (seg, nA = 7, return.loc = FALSE, sizelimit1) 
{
  ...
  for (i in unique(seg[, 1])) {
    ...
    segLOH <- segSamp[segSamp[, nB] == 0 & segSamp[, nA] != 
                        0, , drop = F]
    segLOH <- segLOH[segLOH[, 4] - segLOH[, 3] > sizelimit1, 
                     , drop = F]
...
calc.ai_new <- function (seg, chrominfo, min.size = 1e+06, cont = 0, ploidyByChromosome = TRUE, 
                         shrink = TRUE) 
{
  ...
  ascat.ploidy <- setNames(seg[!duplicated(seg[, 1]), 9], seg[!duplicated(seg[, 
                                                                              1]), 1])
  for (j in samples) {
    sample.seg <- seg[seg[, 1] %in% j, ]
    if (!ploidyByChromosome) {
      ...
      for (k in unique(sample.seg[, 6])) {
        ...
        ploidy <- c(ploidy, setNames(sum(tmp[, 4] - tmp[, 
                                                        3]), k))
      }
      ploidy <- as.numeric(names(ploidy[order(ploidy, decreasing = T)]))[1]
      sample.seg[, 9] <- ploidy
      if (ploidy %in% c(1, seq(2, 200, by = 2))) {
        sample.seg[, "AI"] <- c(0, 2)[match(sample.seg[, 
                                                       7] == sample.seg[, 8], c("TRUE", "FALSE"))]
      }
      if (!ploidy %in% c(1, seq(2, 200, by = 2))) {
        sample.seg[, "AI"] <- c(0, 2)[match(sample.seg[, 
                                                       7] + sample.seg[, 8] == ploidy & sample.seg[, 
                                                                                                   7] != ploidy, c("TRUE", "FALSE"))]
      }
    }
    new.sample.seg <- sample.seg[1, ]
    for (i in unique(sample.seg[, 2])) {
      sample.chrom.seg <- sample.seg[sample.seg[, 2] %in% 
                                       i, , drop = F]
      if (nrow(sample.chrom.seg) == 0) {
        next
      }
      if (ploidyByChromosome) {
        ploidy <- vector()
        for (k in unique(sample.seg[, 6])) {
          tmp <- sample.chrom.seg[sample.chrom.seg[, 
                                                   6] %in% k, ]
          ploidy <- c(ploidy, setNames(sum(tmp[, 4] - 
                                             tmp[, 3]), k))
          ploidy <- ploidy[!names(ploidy) %in% 0]
        }
        ploidy <- as.numeric(names(ploidy[order(ploidy, 
                                                decreasing = T)]))[1]
        sample.chrom.seg[, 9] <- ploidy
        if (ploidy %in% c(1, seq(2, 200, by = 2))) {
          sample.chrom.seg[, "AI"] <- c(0, 2)[match(sample.chrom.seg[, 
                                                                     7] == sample.chrom.seg[, 8], c("TRUE", "FALSE"))]
        }
        if (!ploidy %in% c(1, seq(2, 200, by = 2))) {
          sample.chrom.seg[, "AI"] <- c(0, 2)[match(sample.chrom.seg[, 
                                                                     7] + sample.chrom.seg[, 8] == ploidy & sample.chrom.seg[, 
                                                                                                                             8] != 0, c("TRUE", "FALSE"))]
        }
        sample.seg[sample.seg[, 2] %in% i, 9] <- ploidy
        sample.seg[sample.seg[, 2] %in% i, "AI"] <- sample.chrom.seg[, 
                                                                     "AI"]
      }
      if (class(chrominfo) != "logical") {
        if (sample.chrom.seg[1, "AI"] == 2 & nrow(sample.chrom.seg) != 
            1 & sample.chrom.seg[1, 4] < (chrominfo[i, 
                                                    2])) {
          sample.seg[sample.seg[, 2] == i, "AI"][1] <- 1
        }
        if (sample.chrom.seg[nrow(sample.chrom.seg), 
                             "AI"] == 2 & nrow(sample.chrom.seg) != 1 & 
            sample.chrom.seg[nrow(sample.chrom.seg), 3] > 
            (chrominfo[i, 3])) {
          sample.seg[sample.seg[, 2] == i, "AI"][nrow(sample.seg[sample.seg[, 
                                                                            2] == i, ])] <- 1
        }
      }
      if (nrow(sample.seg[sample.seg[, 2] == i, ]) == 1 & 
          sample.seg[sample.seg[, 2] == i, "AI"][1] != 
          0) {
  ...
  colnames(no.events) <- c("Telomeric AI", "Mean size", "Interstitial AI", 
                           "Mean Size", "Whole chr AI", "Telomeric LOH", "Mean size", 
                           "Interstitial LOH", "Mean Size", "Whole chr LOH", "Ploidy", 
                           "Aberrant cell fraction")
  for (j in samples) {
    sample.seg <- seg[seg[, 1] %in% j, ]
    no.events[j, 1] <- nrow(sample.seg[sample.seg[, "AI"] == 
                                         1, ])
    no.events[j, 2] <- mean(sample.seg[sample.seg[, "AI"] == 
                                         1, 4] - sample.seg[sample.seg[, "AI"] == 1, 3])
    no.events[j, 3] <- nrow(sample.seg[sample.seg[, "AI"] == 
                                         2, ])
    no.events[j, 4] <- mean(sample.seg[sample.seg[, "AI"] == 
                                         2, 4] - sample.seg[sample.seg[, "AI"] == 2, 3])
    no.events[j, 5] <- nrow(sample.seg[sample.seg[, "AI"] == 
                                         3, ])
    no.events[j, 11] <- ascat.ploidy[j]
    no.events[j, 12] <- unique(sample.seg[, 10])
    sample.seg <- sample.seg[sample.seg[, 8] == 0, ]
    no.events[j, 6] <- nrow(sample.seg[sample.seg[, "AI"] == 
                                         1, ])
    no.events[j, 7] <- mean(sample.seg[sample.seg[, "AI"] == 
                                         1, 4] - sample.seg[sample.seg[, "AI"] == 1, 3])
    no.events[j, 8] <- nrow(sample.seg[sample.seg[, "AI"] == 
                                         2, ])
    no.events[j, 9] <- mean(sample.seg[sample.seg[, "AI"] == 
                                         2, 4] - sample.seg[sample.seg[, "AI"] == 2, 3])
    no.events[j, 10] <- nrow(sample.seg[sample.seg[, "AI"] == 
                                          3, ])
  }
  return(no.events)
}
